...
{
    public class ColorByStringConverterTester : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                Debug.Log("text.text: " + text.text + ". len: " + text.text.Length);
                text.text = "LVL" + (Convert.ToInt32(text.name.Replace("TestLabel (", "").Replace(")", "")) + _mode).ToString();
            }
        }
        ...
        private void Update()
        {
            ...
            foreach (TMP_Text text in _texts)
            {
                //text.color = Utils.GetColorByString(_loginLabel.text.Substring(0, _loginLabel.text.Length < 7 ? _loginLabel.text.Length: 7) + GetStringByNumber(Convert.ToInt32(text.text.Replace("LVL", ""))));
                text.color = Utils.GetColorByString(_loginLabel.text + GetStringByNumber(Convert.ToInt32(text.text.Replace("LVL", ""))));
                Debug.Log("_loginLabel.text: " + _loginLabel.text + ". len: " + _loginLabel.text.Length);

                if (_loginLabel.text.Contains("Pavel_1"))
                {
                    for (int i = 0; i < _loginLabel.text.Length; i++)
                    {
                        char c = _loginLabel.text[i];
                        Debug.Log($"[{i}] = '{c}' ({(int)c}, Unicode: U+{(int)c:X4})");
                    }
                }
            }
        }
